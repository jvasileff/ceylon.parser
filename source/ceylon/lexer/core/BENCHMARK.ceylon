...
shared void run() {
    for (i in 0:10) {
        ...
        print((system.nanoseconds - start).nearestFloat / 1000M);
    }
}

String colorTrompon2(String code) {
    ...
    for (token in CeylonTokenIterator(code)) {
        if (token.type.string == "whitespace") {
            ...
            result.append("<Span ``classes(token.type)``>" + token.text + "</span>");
        }
    }
...
String colorTrompon(String code)
    =>  Span {
            ...
            if (token.type.string == "whitespace") then
                ...
                Span {
                    //classNames = classes(token.type);
                    clazz = classes(token.type);
                    token.text
                }
        }.string;

"Return a [[String]] of CSS classes for the given [[TokenType]]. Results are cached so
 that at most one [[String]] is created per [[TokenType]] across multiple invocations of
 this function."
String(TokenType) classes = memoize((TokenType t)
    =>  " ".join({
            t.string,
            (t is KeywordType) then "KeywordType",
            (t is NumericLiteralType) then "NumericLiteralType",
            (t is StringLiteralType) then "StringLiteralType"
        }.coalesced));

"Creates a lazy stream of [[Token]]s for the given Ceylon [[code]]."
...
"[[memoize]] creates a cache for any given [[compute]] function."
